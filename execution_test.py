...
brown = BrownianMotion(1, 1000)   #D
#Ma(brown)


brown_tij = brown.total_movement()

time_seq_array = tij.time_sequence(brown_tij)

timeline_array=tij.timeline(time_seq_array, 1)  #time dt

quantities_brown = tij.quantities_calculator(timeline_array)

tij.compare_quantities([quantities_brown], ['detection-model-r-theta , detect radius=.1, N=1000, steps=20000'],  scale='log')
