...
def compare_quantities(quantities_array, label_array, title='Comparison tij data', scale='linear'):
    ...
    for j, data in reversed(list(enumerate(quantities_array))):
        ...
        for i in range(4):
            ...
            if i==0:
                np.savetxt("contact-duration-brown-detection-D1-R2-N1000-bins.txt", bins)
                np.savetxt("contact-duration-brown-detection-D1-R2-N1000-counts.txt", counts)
            elif i==1:
                np.savetxt("intercontact-duration-detection-brown-D1-R2-N1000-bins.txt", bins)
                np.savetxt("intercontact-duration-detection-brown-D1-R2-N1000-counts.txt", counts)
            elif i==2:
                np.savetxt("number-of-contact-brown-detection-D1-R2-N1000-bins.txt", bins)
                np.savetxt("number-of-contact-brown-detection-D1-R2-N1000-counts.txt", counts)

            else:
                np.savetxt("weight-brown-detection-D1-R2-N1000-bins.txt", bins)
                np.savetxt("weight-brown-detection-D1-R2-N1000-counts.txt", counts)
